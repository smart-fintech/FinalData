...
import os, sys
from detect import detfin
from detectp import detfin
...
class DocScanner1():
    ...
    def interactive_get_contour(self, screenCnt, rescaled_image):
        poly = Polygon(screenCnt, animated=False, fill=False, color="yellow", linewidth=5)
        fig, ax = plt.subplots()
        ax.add_patch(poly)
    ...
    def scanp(self, image_path):

        RESCALED_HEIGHT = 500.0
        OUTPUT_DIR = settings.MEDIA_ROOT +'/priendedoutput'
        print("^^^^^^^^^^^^^^^^^^^^^^^^^6",image_path)
        ...
        # image_path = "./media/images/test1_KgnQgS7.jpeg"
        image = cv2.imread(image_path)
        print("hggggggggggggggggggggg",image)
        ...
        save = name + ".jpg"
        print("saveeeeeeeeeeeeeeeeeeeeeeeeeeee",save)
        image21 = cv2.imwrite(OUTPUT_DIR + '/' + save, warped)
        print("scan_image", image21)
