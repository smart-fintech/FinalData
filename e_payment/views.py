    ...
    def patch(self, request,pk):  
        ...
        if serializer.is_valid():
            ...
            if queryset.EditLegder!='' and queryset.EditLegder2!='':
                y=-(float(request.data['EditLegderamount']))
                queryset.EditLegderamount=y
            else:
                pass
            ...
            if queryset.EditLegder!='' and queryset.EditLegder2!='':
                ...
                # data+='<VOUCHERTYPENAME>Payment</VOUCHERTYPENAME>'
                # data+='<VOUCHERNUMBER>3</VOUCHERNUMBER><ALLLEDGERENTRIES.LIST>'
                # data+='<LEDGERNAME>'+queryset.EditLegder+'</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>'
                # data+='<AMOUNT>'+str(queryset.EditLegderamount)+'</AMOUNT></ALLLEDGERENTRIES.LIST>'
                # data+='<ALLLEDGERENTRIES.LIST><LEDGERNAME>'+str(queryset.EditLegder2)+'</LEDGERNAME>'
                # data+='<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE><AMOUNT>'+queryset.EditLegder2amount+'</AMOUNT></ALLLEDGERENTRIES.LIST>'
                # data+='</VOUCHER></TALLYMESSAGE></DATA></BODY></ENVELOPE>'
                # req = requests.post(url=url, data=data)
                if queryset.ListLegder2!='' and  queryset.ListAmount2!='':
                    list1=[]
                    list2=[]
                    x=queryset.ListLegder2
                    a=x.split(',')
                    y=queryset.ListAmount2
                    b=y.split(',')
                    for a1 in a:
                        list1.append(a1)
                    for b1 in b:
                    ...
                    for i,j in res.items():
                        ...
                        # data+='</VOUCHER></TALLYMESSAGE></DATA></BODY></ENVELOPE>'
                        # req = requests.post(url=url, data=data)
            elif queryset.ListLegder1!='' and  queryset.ListAmount1!='':
                list1=[]
                list2=[]
                x=queryset.ListLegder1
                a=x.split(',')
                y=queryset.ListAmount1
                b=y.split(',')
                for a1 in a:
                    list1.append(a1)
                for b1 in b:
...
class Newvoucherpost(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data+='<BODY><DESC><STATICVARIABLES><SVCURRENTCOMPANY>'+str(model.bank.bankname.companyname)+'</SVCURRENTCOMPANY></STATICVARIABLES></DESC><DATA>'
        data+='<TALLYMESSAGE><VOUCHER><DATE>'+str(newdate)
        data+='</DATE><NARRATION>'+model.Transaction
        data+='</NARRATION><VOUCHERTYPENAME>Payment</VOUCHERTYPENAME>'
        data+='<ALLLEDGERENTRIES.LIST><LEDGERNAME>'+model.Legder
        data+='</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>'
        data+='<AMOUNT>'+str(model.Debit)
        data+='</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST>'
        data+='<LEDGERNAME>'+str(model.bank)
        data+='</LEDGERNAME><ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>'
        data+='<AMOUNT>'+str(model.Credit)
